...
void queuesFree(t_queues * queues) {
	...
	for (i = (unsigned long) listBegin(queues->activeNodes);
		...
		listFreeWithData(queues->packets[i-1]);
...
void queuesAddPacket(t_queues * queues, t_packet * packet, int node) {
	...
	if (listLength(queues->packets[node]) == queues->queueLimit) {

		for (last = listEnd(queues->packets[node]); last; last = listPrev(queues->packets[node])) {

			if (last->currentHop) {

				listDelCurrent(queues->packets[node]);
				free(last);
				break ;
			}
		}
...
//printf("Not in its first hop\n");
		...
		if (prevHopP2 != nextHopP1 && graphGetCost(graph, prevHopP2, nextHopP1) > 1.0 / 0.8) continue ;
//printf("Previous hop match 1\n");

		nextHopP2 = (unsigned long) arrayGet(arrayGet(paths, p->flow), p->currentHop + 1);
		if (prevHopP1 != nextHopP2 && graphGetCost(graph, prevHopP1, nextHopP2) > 1.0 / 0.8) continue ;
...
//printf("%ld ", (long) arrayGet(arrayGet(paths, i), j));
			...
			tmp = 1 - 100.0/graphGetCost(graph, lastNode, node);
			if (tmp > 0)
