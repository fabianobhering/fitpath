...
#define CONFLICT_LIMIAR		(GRAPH_MULTIPLIER / 0.001)
...
//printf("At time %d, Packet just arrived at hop %d for flow %d with probability %f\n", time, packet->currentHop, packet->flow, packet->deliveryProbability);
				/*
...
#define MAX_NUMBER_OF_STATES	1000
			...
			if (i == numberOfFlows) {
				...
				for (i = 0; i < numberOfFlows; i++) arraySet(deliveredPacketsFlows, i, 0);
printf("Closing cicle %d at time %d with %f delivered packets\n", lookAhead, time, deliveredPackets);
				if (lookAhead > 1) {
