...
printf("%ld ", (long) arrayGet(arrayGet(paths, i), j));
			tmp = 1 - 100.0/graphGetCost(graph, (long) arrayGet(arrayGet(paths, i), j - 1),(long) arrayGet(arrayGet(paths, i), j));
			if (tmp > 0)
...
printf("CorrectionFactor = %f\n", correctionFactor);
			meanInterval = ((float) (stateGetCurrentTime(state) - stateGetCurrentTime(oldState))) /
				(correctionFactor * ((float) (stateGetDeliveredPackets(state) - stateGetDeliveredPackets(oldState))));
