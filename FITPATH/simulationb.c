...
t_graph * simulationConflictGraph(t_graph * graph, t_array * paths, int * linkIndexBase) {
	...
	for (i = 0; i < arrayLength(paths); i++) {
		...
		for (j = 0; j < arrayLength(path1) - 1; j++) {
			...
			for (k = 0; k < arrayLength(paths); k++) {

				path2 = arrayGet(paths, k);
				for (l = 0; l < arrayLength(path2) - 1; l++) {
...
/*
 * The following constant represents the maximum number of 
 * delivered packets we'll generate trying to find stead-state during 
 * the simulation. 
 */
#define MAX_SIM_DELIVERED_PACKETS 500
...
float simulationSimulate(t_graph * graph, t_array * paths) {
	...
	t_array * path;
	t_list * neighbors;
	t_list * onBufferPackets, * onTransmissionPackets;
...
//graphPrint(conflict);
	...
	/*
	 * We'll have a list for packets on 
	 * buffer and a list for packets on transmission.
	 */
	onBufferPackets = listNew();
	...
	for (i = 0; i < numberOfFlows; i++) {
		...
		if (arrayGet(blockedLinks, simulationConflictNodeIndex(linkIndexBase, i, 0))) {
			...
			/*
			 * Also add it to the onBufferPackets list,
			 * so that we know this packet is disputing
			 * the wireless medium.
			 */
			listAdd(onBufferPackets, newPacket);
		}
		else {
	 ...
	 * 2) Loop through the onBufferPackets list trying to 
	...
	while(1) {
		...
		for (packet = listBegin(onTransmissionPackets); packet; packet = listNext(onTransmissionPackets)) {
			...
			if (packet->ETA <= 0) {
				...
				if (packet->currentHop == arrayLength(arrayGet(paths, packet->flow)) - 1) {
					...
					/*
					 * Add the packet to the node's queue and to
					 * the onBufferPackets list.
					 */
					queuesAddPacket(queues, packet, (long) arrayGet(arrayGet(paths, packet->flow), packet->currentHop));
					listAdd(onBufferPackets, packet);
				}
			}
			else {
		...
		/*
		 * Now we loop through the list of the packets on buffer 
		 * to see if we can transmit them.
		 */
		for (packet = listBegin(onBufferPackets); packet; packet = listNext(onBufferPackets)) {
...
//printf("time %.2f: transmiting packet between %d and %d\n", time, (long) arrayGet(path, packet->currentHop), (long) arrayGet(path, packet->currentHop + 1));
			/*
			 * Move the packet from the onBuffer list to 
			 * the onTransmission list.
			 */
			listAdd(onTransmissionPackets, packet);
			listDelCurrent(onBufferPackets);
			...
			if (codedPacket) {
				 ...
				 */
				listAdd(onTransmissionPackets, packet);
				listDel(onBufferPackets, packet);
		...
		if (deliveredPackets > MAX_SIM_DELIVERED_PACKETS) {

			meanInterval = ((float) time) / ((float) deliveredPackets);
//printf("*************Failed to find stead-state. Using %.2f as estimate (%llu/%d)\n", meanInterval, time, deliveredPackets);
			break ;
		}
	}

	/*
	 * TODO: free a bunch of memory.
	 */
	...
	queuesFree(queues);
	free(queues);
	listFreeWithData(onBufferPackets);
	free(onBufferPackets);
