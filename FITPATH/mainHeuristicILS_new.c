...
void printCurrentPaths(t_array * paths, int numberOfPairs, int numberOfDescriptors){  
    ...
    if (isFirstSolution == 0) {
    FILE *arq_ns3;
    char arq_name[50];
	sprintf(arq_name,"../inst/newILS/0new-route300_%d-%d-ref%d-1", NumNodes, INST, REF);
...
void printDSR(t_array * paths[], int numberOfPairs, int numberOfDescriptors, t_return * rf, clock_t start ){  
    printf("DSR Routes\n");
    //INST++;
    FILE *arq_ns3, *arq_result;
    char arq_name[50];
	sprintf(arq_name,"../inst/newILS/0new-route300_%d-%d-ref%d-2", NumNodes, INST, REF);
    ...
    char arq_name2[50];
	sprintf(arq_name2,"../inst/newILS/time-route300_%d-%d-ref%d", NumNodes, INST, REF);

	arq_ns3 = fopen(arq_name2, "w");
...
int main(int argc, char ** argv) {
	...
	clock_t start = clock(); //adicionado por debora em 09/11/2023
...
//**************************************************************
	...
	*/
    ...
    //r->start = start; // Armazenar o tempo inicial adicionado por debora em 09/11/2023
    ...
    // Calcular o tempo decorrido usando a função difftime
    //double elapsedTime = difftime(time(NULL), start);//adicionado por debora em 09/11/2023
...
printDSR(bestPaths, numberOfPairs, numberOfDescriptors, rf, start);
//printSolution(currentPaths, numberOfPairs, numberOfDescriptors, graph);
