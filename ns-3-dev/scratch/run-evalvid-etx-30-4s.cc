...
const std::string baseDirLinks = "../../inst/links/hall-30/w-htc/4s-256/";
const std::string baseDirInputFitpath = "../inst/links/hall-30/w-htc/4s-256/";
const std::string baseDirEvalvid = "../../inst/evalvid/hall-30/w-htc/4s-256/";
...
void RecallEtx (int transmittingSources, int numNodes, int inst, int refId){
    ...
    if (file0.is_open()) {
        ...
        std::ifstream file1 ("config-initialization");
        if (file1.is_open()){
            std::string line;
            ...
            std::cerr << "Error opening file1 config-initialization" << std::endl;
        }
        file0.close();  
    } else {
...
void CallInputFitPath (int numNodes, int inst, int refId, NodeContainer adhocNodes, Ipv4InterfaceContainer adhocInterfaces, uint16_t pktSizeEval){ 
    ...
    std::ifstream file2("config-initialization"); 
    ...
    if (file2.is_open()){
        ...
        std::cerr << "Error opening file2 config-initialization" << std::endl;
    }
   ...
   //PrintIntermediateTC(int numNodes);
    ...
    for (uint32_t j=0; j< sources.size(); j++){
        ...
        pathsFileName.str("");
        pathsFileName << "../../inst/newILS/0new-route300_" << numNodes << "-" << inst << "-ref" << refId;
...
int main(int argc, char *argv[])
{
    ...
    float FitpathCallTime = 300.0;
    float TotalTime = 340.0;
...
initialAlloc->Add (Vector (0 , 150, 3));
initialAlloc->Add (Vector (0 , 0, 3));
//initialAlloc->Add (Vector (2 , 234, 3));
...
//initialAlloc->Add (Vector (116 , 129, 3));
