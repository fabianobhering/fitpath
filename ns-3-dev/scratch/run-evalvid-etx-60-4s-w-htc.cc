...
const std::string baseDirLinks = "../../inst/links/coastguard-60/w-htc/4s-3M/";
const std::string baseDirInputFitpath = "../inst/links/coastguard-60/w-htc/4s-3M/";
const std::string baseDirEvalvid = "../../inst/evalvid/coastguard-60/w-htc/4s-3Ma/";
...
void RecallEtx (int transmittingSources, int numNodes, int inst, int refId){
    ...
    if (file0.is_open()) {
        ...
        std::ifstream file1 ("config-initialization-w-htc");
        if (file1.is_open()){
            std::string line;
            ...
            std::cerr << "Error opening file1 config-initialization-w-htc" << std::endl;
        }
        file0.close();  
    } else {
...
void CallInputFitPath (int numNodes, int inst, int refId, NodeContainer adhocNodes, Ipv4InterfaceContainer adhocInterfaces, uint16_t pktSizeEval){ 
    ...
    std::ifstream file2("config-initialization-w-htc"); 
    ...
    if (file2.is_open()){
        ...
        std::cerr << "Error opening file2 config-initialization-w-htc" << std::endl;
    }
...
initialAlloc->Add (Vector (300 , 300, 3));
initialAlloc->Add (Vector (0 , 100, 3));
initialAlloc->Add (Vector (100 , 0, 3));//initialAlloc->Add (Vector (150 , 0, 3));//na simulação de 30 nós ficou com x=100 e na simulação de 60 nós alterei x para 150
initialAlloc->Add (Vector (50 , 50, 3));
initialAlloc->Add (Vector (0 , 150, 3));
initialAlloc->Add (Vector (209 , 297, 3));// comentar e excluir essa coordenada nas simulações de 30 nós
...
initialAlloc->Add (Vector (300 , 300, 3));
initialAlloc->Add (Vector (0 , 100, 3));
initialAlloc->Add (Vector (100 , 0, 3)); //initialAlloc->Add (Vector (200 , 0, 3));// na simulação de 30 nós ficou com x=100 e na simulação de 60 nós alterei x para 150
initialAlloc->Add (Vector (50 , 50, 3));
