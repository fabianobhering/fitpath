...
EvalvidServer::Setup()
{
  ...
  while (videoTraceFile >> frameId >> frameType >> frameSize >> numOfUdpPackets >> sendTime)
    {
      ...
      //NS_LOG_UNCOND(">> EvalvidServer: " << frameId << "\t" << frameType << "\t" <<
      //                          frameSize << "\t" << numOfUdpPackets << "\t" << sendTime << "\t" << Seconds(sendTime - lastSendTime));
  ...
  //Create CBR TRAFFIC
  //st-bus1m: 738; st-bus_loop1min: 8422
/*
  for (frameId=4; frameId<=8422;frameId++)
    {
      videoInfoStruct = new m_videoInfoStruct_t;
      videoInfoStruct->frameType = "I";
      videoInfoStruct->frameSize = 1024;
      videoInfoStruct->numOfUdpPackets = 1;
      videoInfoStruct->packetInterval = Seconds(0.009102222);   
      m_videoInfoMap.insert (pair<uint32_t, m_videoInfoStruct_t*>(frameId, videoInfoStruct));

      videoInfoStruct = new m_videoInfoStruct_t;
      videoInfoStruct->frameType = "P";
      videoInfoStruct->frameSize = 1024;
      videoInfoStruct->numOfUdpPackets = 1;
      //videoInfoStruct->packetInterval = Seconds(0.010922667); 
      videoInfoStruct->packetInterval = Seconds(0.009102222);      
      m_videoInfoMap.insert (pair<uint32_t, m_videoInfoStruct_t*>(frameId, videoInfoStruct));
    }
*/
  ...
  if (m_videoInfoMapIt != m_videoInfoMap.end())
    { 
      ...
      for(int i=0; i<m_videoInfoMapIt->second->numOfUdpPackets - 1; i++)
        ...
        }else if(REF==4 || REF==5 || REF==6 || REF==7){
          //std::cout <<"MDC " << REF << "\n";
